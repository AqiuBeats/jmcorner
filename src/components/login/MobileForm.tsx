import { Button, Col, Form, Input, Row } from 'antd'; //Statistic
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Input placeholder={'sys.login.mobile'} />
        </Form.Item>
        <Form.Item
          name="code"
          rules={[{ required: true, message: 'sys.login.mobilePlaceholder' }]}
        >
          <Row justify="space-between">
            <Col span={14}>
              <Input placeholder={'sys.login.smsCode'} />
            </Col>
            <Col span={9} flex={1}>
              {/* <Button
							...
							>
								{countdown === 0 ? (
									<span>{("sys.login.sendSmsButton")}</span>
								) : (
